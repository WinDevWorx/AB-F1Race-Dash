...
import PitCrewView from '../pitcrew/PitCrewView';
...
export default function TeamRacingContainer({ data }) {
  ...
  // Auto-cycling logic: Top 5 Consultants (5s) → Monaco (5s) → Kyalami (5s) → Championship (5s + scroll) → Scroll Down (6s) → Scroll Up (6s) -> PitCrewView (60s) → Repeat
  useEffect(() => {
            ...
            cycleTimeoutRef.current = setTimeout(() => {
              ...
              cycleTimeoutRef.current = setTimeout(() => {
                ...
                scrollTimeoutRef.current = setTimeout(() => {
                  ...
                  scrollTimeoutRef.current = setTimeout(() => {
                    // PitCrewView - 3 seconds, then scroll
                    setActiveCircuit("PitCrew");
                    setScrollPosition("top");
                    console.log("Auto-cycle: Pit Crew View - 3s");

                    cycleTimeoutRef.current = setTimeout(() => {
                      // Scroll Down - 90 seconds
                      console.log("Auto-cycle: Pit Crew scrolling down - 90s");
                      const startPosition = window.pageYOffset;
                      const targetPosition = document.body.scrollHeight - window.innerHeight;
                      const distance = targetPosition - startPosition;
                      ...
                      function smoothScrollDownPitCrew(timestamp) {
                        if (!start) start = timestamp;
                        const progress = timestamp - start;
                        const percentage = Math.min(progress / duration, 1);
                        ...
                        if (progress < duration) {
                          requestAnimationFrame(smoothScrollDownPitCrew);
                        }
                      }
                      ...
                      scrollTimeoutRef.current = setTimeout(() => {
                        // Scroll Up - 90 seconds
                        console.log("Auto-cycle: Pit Crew scrolling up - 90s");
                        const startPos = window.pageYOffset;
                        const targetPos = 0;
                        const dist = startPos - targetPos;
                        const dur = 900000; // 90 seconds
                        let startTime = null;

                        function smoothScrollUpPitCrew(timestamp) {
                          if (!startTime) startTime = timestamp;
                          const prog = timestamp - startTime;
                          const perc = Math.min(prog / dur, 1);
                          ...
                          if (prog < dur) {
                            requestAnimationFrame(smoothScrollUpPitCrew);
                          }
                        }
                        ...
                        scrollTimeoutRef.current = setTimeout(() => {
                          // Repeat cycle
                          cycle();
                        }, 950000); // 90s scroll + 2s buffer
                      }, 950000); // 90s scroll + 2s buffer
                    }, 3000); // 3 seconds initial display
                  }, 8000);
                }, 8000);
              }, 5000);
            }, 5000);
...
function TopConsultantsView({ consultants }) {
  ...
  const getPitCrewImage = (consultant, index) => {
    ...
    const pitMember = pitMemberImage.find(
      pit => pit.pitMemberName.toLowerCase() === consultant.consultantName?.toLowerCase()
    );
    ...
    return pitMember?.pitMemberImage || fallbackPitMember?.pitMemberImage || "/OT_IMG.png";
  };
  ...
  return (
    <div className={styles.container}>
      <div className={styles.circuitTabs}>
        <button
          className={`${styles.tabButton} ${activeCircuit === 'TopConsultants' ? styles.active : ''}`}
        ...
        {/* ADDING NEW View */}
          <button
          className={`${styles.tabButton} ${activeCircuit === 'PitCrew' ? styles.active : ''}`}
          onClick={() => handleManualTabClick('PitCrew')}
          data-testid="PitCrew"
        >
          🏆 Pit Crew View
        </button>
