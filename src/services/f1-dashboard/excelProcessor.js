...
export class ExcelProcessor {
  ...
  static transformData(rawData) {
    ...
    rawData.forEach((row, index) => {
      const consultantName = this.getColumnValue(row, columnMappings.consultant_name) || `Consultant ${index + 1}`;
      const supervisorName = this.getColumnValue(row, columnMappings.supervisor_name) || 'Unknown Supervisor';
      const teamName = this.getColumnValue(row, columnMappings.team_name) || 'Unknown Team';
      const currentSalesRaw = this.getColumnValue(row, columnMappings.current_sales);
      const salesTargetRaw = this.getColumnValue(row, columnMappings.sales_target);
      
      const currentSales = parseFloat(currentSalesRaw) || 0;
      const salesTarget = parseFloat(salesTargetRaw) || 0;
      
      if (index < 3) { // Debug first few records
    ...
    for (const [field, possibleNames] of Object.entries(ExcelColumnMappings)) {
      for (const column of columns) {
        if (possibleNames.some(name => column.toLowerCase().includes(name.toLowerCase()))) {
          ...
          console.log(`Mapped ${field} to column: ${column}`);
          break;
        }
      }
      if (!mappings[field]) {
