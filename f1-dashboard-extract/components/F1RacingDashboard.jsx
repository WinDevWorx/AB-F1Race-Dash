...
export default function F1RacingDashboard({ excelPath = '/Book1.xlsx' }) {
  ...
  const processExcelData = (rawData) => {
    ...
    rawData.forEach((row, index) => {
      const supervisorName = row['Supervisor Name'] || row.SupervisorName || row.supervisor;
      const consultantName = row['Consultant Name'] || row.ConsultantName || row.consultant;
      
      if (!supervisorName || !consultantName) return;
      
      const salesActual = parseFloat(row['TotalSalesVal'] || row.TotalSalesVal || 0);
      const salesTarget = parseFloat(row['SalesValTarget'] || row.SalesValTarget || 0);
      const appsActual = parseInt(row['TotalRealAppsVol'] || row.TotalRealAppsVol || 0);
      const appsTarget = parseInt(row['RealAppsTarget'] || row.RealAppsTarget || 0);
      ...
      <div className="flex items-center justify-center h-screen bg-gradient-to-br from-slate-900 to-slate-800">
        <div className="text-center max-w-md p-8 bg-red-900/20 border border-red-500/30 rounded-lg">
          ...
          <p className="text-sm text-gray-400">
            Make sure Book1.xlsx is placed in your public folder and contains the required columns:
            Supervisor Name, Consultant Name, TotalSalesVal, SalesValTarget, TotalRealAppsVol, RealAppsTarget
          </p>
        </div>
      </div>
    );
  ...
  console.log('F1RacingDashboard: Passing data to MainDashboard component:', data);
